...
class Scatter(MessagePassing):
    def __init__(self, in_channels, out_channels, lamb,V,dropout_prob=0.5, if_mlp=True, head=2,layer=3,J=3,learn=True):
        ...
        self.learn = learn
        num_gat = int((3**layer-1)/2)#layer*(J-1) ####multiple times1+(3-1)+(3-1)*3=9,low pass1,2,4,8,16high pass2,4,8,16,32
        if self.learn==True:
            self.conv_list = nn.ModuleList([GATConv(in_channels, out_channels, heads=head, dropout=dropout_prob) for i in range(0,num_gat)])
            self.mlp_list = nn.ModuleList([Seq(ELU(),Dropout(dropout_prob),Linear(head * out_channels, out_channels),nn.BatchNorm1d(out_channels)) for i in range(0,num_gat)])
        self.lamb=lamb
        self.V= V
        self.layer=layer
        if self.learn==False:
            self.mlp_learn = nn.ModuleList([Seq(nn.BatchNorm1d(in_channels),ELU(), Dropout(dropout_prob), Linear(in_channels, out_channels)) for i in range(0, num_gat)])
        self.highpass=False
    ...
    def forward(self, x_in, edge_index):
        ...
        if self.learn==True:
            ...
            x_out.append(self.mlp_learn[0](x.pop(0)))
        ...
        for i in range(1,self.layer):  ###layer-1
            for k in range(len(x_next)):    ###gat_num=3,6,12,len=2,4,8
                ...
                if self.learn == True:
                    ...
                    x_out.append(self.mlp_learn[index](x.pop(0)))
                index+=1
...
class Net(nn.Module):
    def __init__(self, num_features, nhid, num_classes, lamb,V, dropout_prob=0.5, shortcut=True, if_mlp=True, head=2,layer=3,learn=False):
        super(Net, self).__init__()
        self.shortcut = shortcut
        self.scatter = Scatter(num_features, nhid, lamb,V,dropout_prob=dropout_prob, if_mlp=if_mlp, head=head,layer=layer,learn=learn)
        self.mlp = Seq(
            nn.BatchNorm1d(int((3**layer-1)/2)*nhid),
            ELU(),
            Dropout(dropout_prob),
...
def objective(learning_rate=0.01, weight_decay=0.01, nhid=64, dropout_prob=0.5, if_mlp=True, head=2,
    ...
    PATH = "/home/jyh_temp1/Downloads/scatter_MP/GFA-main/exp_result/acc"+str(round(np.mean(saved_model_test_acc),2))+\
...
if __name__ == '__main__':
    ...
    parse.add_argument("action", type=str, default=ray_train, help="ray or arg train")
