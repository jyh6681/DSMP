...
class SepNet(nn.Module):
    def __init__(self, num_features, nhid, num_classes, dropout_prob, head=4,layer=2,if_gat=True,if_linear=True):
        ...
        if self.if_linear:
            print("using linear transform")
            self.lin_in = Linear(num_features, nhid, bias=False)
        else:
            print("without using linear transform")
            nhid=num_features
...
def objective(learning_rate=0.01, weight_decay=0.01, nhid=64, dropout_prob=0.5, num_epochs = 200,if_gat=True, if_linear=False,head=4,
    ...
    for rep in range(num_reps):
        ...
        model = SepNet(num_features, nhid, dataset.num_classes, dropout_prob, head=head,layer=2,if_gat=if_gat,if_linear=if_linear).to(device)
...
def ray_train():
    ...
    analysis = tune.run(
        ...
        config={
            ...
            "learning_rate": tune.grid_search([1e-3,5e-3,1e-4]),
            ...
            "dropout_prob": tune.grid_search([0.1,0.5]),
