...
class SVDQM7(InMemoryDataset):
    ...
    def process(self,layer=2):
        ...
        for i in range(target.shape[0]):
            ...
            scatter_list = diffusion_scat(edge_index)

            edge_attr = coulomb_matrix[i, edge_index[0], edge_index[1]]
            y_origin = target[i].item()
            y = (target[i].item() - mean) / std
            data = Data(x= x,edge_index=edge_index, edge_attr=edge_attr, y=y,scatter_list=scatter_list)
            data.num_nodes = edge_index.max().item() + 1
            data.y_origin = y_origin
            data_list.append(data)
...
def MyDataset(dataset, num_features, QM7=True):
    ...
    for i in range(len(dataset)):
        if QM7:
            ...
            data1 = Data(x=dataset[i].x, edge_index=dataset[i].edge_index, y=dataset[i].y,scatter_list=dataset[i].scatter_list)####pyG dataset
...
def qm_test(model, loader, device, mean, std,epoch,dataname="qm7"):
    model.eval()
    loss = 0.
    for data in loader:
        data = data.to(device)
        ...
        out = out * std + mean
        if dataname=="qm7":
            loss += F.l1_loss(out, data.y_origin, reduction='mean').item()
        else:
            loss += F.l1_loss(out, data.y, reduction='mean').item()
        if dataname=="QM9":
            loss_all = F.l1_loss(out,data.y, reduction='mean',reduce=True,size_average=True)
            print("loss_all in epoch:",epoch,"is ",loss_all,loss_all.shape)

    return loss / len(loader.dataset)


def qm_test_train(model, loader, device):
    model.eval()
    loss = 0.
    for data in loader:
        data = data.to(device)
        out = model(data)
        loss += F.mse_loss(out, data.y, reduction='mean').item()
    return loss / len(loader.dataset)
...
class SVDTUD(InMemoryDataset):
    ...
    def process(self):
        ...
        for data in data_list:
            ...
            x_in = data.x
            print("shape x_in:",x_in.shape)
            scatter_list = diffusion_scat(edge_index)
            #print("max:",np.max(scatter_list[0]),np.max(scatter_list[1]),np.max(scatter_list[2]),)
            data.scatter_list=scatter_list### = Data(x= x_in,edge_index=edge_index, y=data.y,scatter_list=scatter_list)
        ...
        if self.pre_filter is not None or self.pre_transform is not None:
            ...
            if self.pre_filter is not None:
                ...
                for data in data_list:
                    ...
                    scatter_list = diffusion_scat(edge_index)
                    data = Data(x=x_in, edge_index=edge_index, y=data.y, scatter_list=scatter_list)
                    new_data_list.append(data)
