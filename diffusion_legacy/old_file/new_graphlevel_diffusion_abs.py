...
class SepNet3(nn.Module):
    def __init__(self, num_features, nhid, num_classes, dropout_prob, hid_fc_dim, pRatio, head=4, layer=3, if_gat=True,
                 if_linear=True):
    ...
    def forward(self, data):
        x_batch, edge_index_batch, batch = data.x, data.edge_index, data.batch  ###scatter should be [2,N]indice,[N,1]value
        print("batch:",x_batch.shape,edge_index_batch.shape,batch.shape,batch)
        scatter_edge_index, scatter_edge_attr = data.scatter_index,data.scatter_value  ##load scatter transform matrix
        _, graph_size = torch.unique(batch, return_counts=True)
        # print("x_batch:", len(scatter_edge_index),len(scatter_edge_attr),scatter_edge_index[0].shape,scatter_edge_attr[0].shape)
        names = locals()
        x_low = self.solo_pass(scatter_edge_index[0], x=x_batch, edge_attr=scatter_edge_attr[0])  ##low pass
        for i in range(1,4):  ###psi={0,1,2}
            names['x_h%s' % i] = torch.abs(self.solo_pass(scatter_edge_index[i], x=x_batch, edge_attr=scatter_edge_attr[i]))
            names['x%s' % i] = self.solo_pass(scatter_edge_index[0], x=names['x_h%s' % i],edge_attr=scatter_edge_attr[0])
            for j in range(1,4):
                names['x%s_h%s'%(i,j)]=torch.abs(self.solo_pass(scatter_edge_index[j], x=names['x_h%s'%i], edge_attr=scatter_edge_attr[j]))
                names['x%s_%s' % (i, j)] = self.solo_pass(scatter_edge_index[0], x=names['x%s_h%s'%(i,j)], edge_attr=scatter_edge_attr[0])

        if self.gat == True:
            x_low = self.mlp_list[0](self.conv_list[0](x_low, edge_index_batch))
            index=1
            for i in range(1,4):
                names['x%s' % i] = self.mlp_list[index](self.conv_list[index](names['x%s' % i], edge_index_batch))
                index += 1
                for j in range(1,4):
                    names['x%s_%s' % (i, j)] = self.mlp_list[index](self.conv_list[index](names['x%s_%s' % (i, j)], edge_index_batch))
                    index+=1

        else:
            x_low = self.mlp_learn[0](x_low)
            index = 1
            for i in range(1, 4):
                names['x%s' % i] = self.mlp_learn[index](names['x%s' % i])
                index += 1
                for j in range(1, 4):
                    names['x%s_%s' % (i, j)] = self.mlp_learn[index](names['x%s_%s' % (i, j)])
                    index += 1
        x_stage1 = x_batch + x_low
        x_stage2=x_stage1
        for i in range(1,4):
            x_stage2 +=names['x%s' % i]
        x_stage3=x_stage2
        for i in range(1, 4):
            for j in range(1, 4):
                x_stage3 += names['x%s_%s' % (i, j)]

        h_pooled = self.global_add_pool(x_stage3, batch)  # grasspool(hidden_rep, graph_size, self.pRatio)
        x = self.fc(h_pooled)
        if self.num_classes == 1:
            return x.view(-1)
        else:
...
class SepNet3Prune(nn.Module):
    def __init__(self, num_features, nhid, num_classes, dropout_prob, hid_fc_dim, pRatio, head=4, layer=3, if_gat=True,
                 if_linear=True):
        super(SepNet3Prune, self).__init__()
        self.layer=layer
        self.num_classes = num_classes
        self.dropout_prob = dropout_prob
        self.nhid = nhid
...
def objective(learning_rate=0.01, weight_decay=0.01, nhid=64,fc_dim=[64, 16],epochs=100, dropout_prob=0.5,batch_size=1024,num_reps=1,\
    ...
    path = osp.join("./", 'data', "diffusion_data4.9", dataname)
    if dataname == 'qm7':
        dataset = SVDQM7(path)
        num_features = 5
        num_classes = 1
    ...
    for r in range(num_reps):
        ...
        model = SepNet3(num_features, nhid, num_classes, dropout_prob, fc_dim, pRatio, head=head,if_gat=if_gat,
        ...
        # Test
        print("****** Test start ******")
        model = SepNet3(num_features, nhid, num_classes, dropout_prob, fc_dim, pRatio, head=head,if_gat=if_gat,if_linear=if_linear).to(device)
...
def ray_train():
    ...
    analysis = tune.run(
        training_function,
        config={
            "dataname": tune.grid_search(["qm7","ogbg-molhiv","NCI1","ZINC_full","IMDB-MULTI","DD"]),#DD,ogbg-molhiv,"Mutagenicity","NCI1","ZINC_full","IMDB-MULTI"]),#DD, Mutagenicity,"NCI1",'Cora', 'PubMed']),  # for ray tune, need abs path ogbg-molhiv
            "learning_rate": tune.grid_search([5e-3]),
            "weight_decay": tune.grid_search([1e-3]),
            "if_gat": tune.grid_search([False]),
            "if_linear": tune.grid_search([False]),
            "batch_size":tune.grid_search([5120]),
