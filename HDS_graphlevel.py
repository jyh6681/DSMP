...
from dataset_src.Diffusion_Graph_Dataset import *
import math
...
os.environ["CUDA_VISIBLE_DEVICES"] = "2,1,7"
config = {"J":3,"L":3,"sct_type":"diffusion","nhid":64}
...
def objective(learning_rate=0.01, weight_decay=0.01, nhid=64,fc_dim=[64, 16],num_scatter=3,epochs=100, dropout_prob=0.5,batch_size=1024,num_reps=1,\
    ...
    path = osp.join("/home/jyh_temp1/Downloads/scatter_MP/ScatteringMP/", 'data', "pure_diffusion", dataname)
    if dataname == 'qm7':
        dataset = SVDQM7(path)
        num_features = 5
        num_classes = 1
    ...
    for r in range(num_reps):
        ...
        model = HDSGNN(num_features, num_classes).to(device)
    ...
    print("test:",test_acc,args.dataname)
    if dataname=="ogbg-molhiv":
        return auc
    else:
...
def ray_train(args):
    ...
    analysis = tune.run(training_function,
        # scheduler=sched,
        config={
            "dataname": tune.grid_search(["COLLAB","NCI1","Mutagenicity","PROTEINS","DD"]), #,,",,"qm7","ogbg-molhiv","NCI1", DD,ogbg-molhiv,"NCI1","ZINC_full","IMDB-MULTI"]),#DD, Mutagenicity,"NCI1",'Cora', 'PubMed']),  # for ray tune, need abs path ogbg-molhiv
            ###TUdataset "ENZYMES","MUTAG","IMDB",NCI1", "DD"
            "learning_rate": tune.grid_search([1e-2,1e-3,1e-4]),
            "weight_decay": tune.grid_search([1e-3,1e-4]),
            "batch_size":tune.grid_search([15,64]),#args.batch_size]),
