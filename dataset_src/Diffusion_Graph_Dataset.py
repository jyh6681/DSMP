...
from model.HDS_Model import get_hyper
from dataset_src.dst import LRSPreprocess
global config
...
class SVDTUD(InMemoryDataset):
    ...
    def process(self):
        ...
        print("shape1:",data_list[0],data_list[1])
        if self.pre_filter is None and self.pre_transform is None:
            new_data_list = LRSPreprocess(data_list, get_hyper().J, get_hyper().L, get_hyper().sct_type)
        ...
        data_list = [self.get(idx) for idx in range(len(self))]
        print("processed:",data_list[0],data_list[1])
