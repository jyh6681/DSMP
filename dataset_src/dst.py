...
def LRSPreprocess(datalist, J, L=-1, scattering_type = "diffusion"):
  ...
  for i in datalist:
    ...
    if L > 0:
      ...
      #turn feature a list to store
      feat_list = []
      for index in range(i.features.shape[0]):
        feat_list.append(i.features[index])
      tmp_data = Data(x=i.x, edge_index=i.edge_index, y=i.y,diff_feat = feat_list)
    datalist_.append(tmp_data)
  return datalist_
