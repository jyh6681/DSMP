...
class BaseNet(nn.Module):
    def __init__(self, num_features, nhid, num_classes, dropout_prob, hid_fc_dim, num_layer, head=4,Conv_type="GPS"):
    ...
    def forward(self, data):
        ...
        # print("batch:",data,graph_size,data.scatter_edge_attr[0].shape)
        if self.if_linear == True:
            x_batch = self.lin_in(x_batch)
        hidden_rep = x_batch
        for index in range(self.num_scatter):
            hidden_rep += self.scatter_nn_list[index](hidden_rep,scatter_edge_index,scatter_edge_attr,edge_index_batch)
