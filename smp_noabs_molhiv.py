...
def test(model, loader, device,dataname):
    ...
    for data in loader:
        ...
        if dataname=="ogbg-molhiv":
            ...
            loss += torch.nn.BCELoss()(out.squeeze(), data.y.float()).item()
            auc_roc.update(input=out.squeeze(), target=data.y)
            # try:
            #     batch+=1
            #     auc+=roc_auc_score(y_true=true,y_score=pred.cpu().numpy(),average="macro")
            ...
            true = data.y.cpu().numpy()#F.one_hot(data.y).cpu().numpy()
            auc= 0
            batch = 1
    auc = round(auc_roc.compute().detach().cpu().item(),4)
    print("auc:",auc)
    return correct / len(loader.dataset), loss / len(loader.dataset),auc#auc/batch
...
def objective(learning_rate=0.01, weight_decay=0.01, nhid=64,fc_dim=[64, 16],num_scatter=3,epochs=100, dropout_prob=0.5,batch_size=1024,num_reps=1,\
    ...
    if dataname == 'qm7':
        ...
        if dataname=="ogbg-molhiv":
            ...
            # loss_criteria = F.cross_entropy #mod in 23.11.1
    ...
    for r in range(num_reps):
        ...
        for epoch in range(epochs):
            ...
            for i, data in enumerate(train_loader):
                ...
                elif dataname=="ogbg-molhiv":
                    out = torch.sigmoid(out)
                    loss = loss_criteria(out.squeeze(), data.y.float())
                else:
...
def ray_train(args):
    ...
    asha_scheduler = ASHAScheduler(
        ...
        brackets=1)
    if args.single_data:
        default_name_list = [args.dataname]
    else:
        default_name_list = ["Mutagenicity","NCI1","DD","PROTEINS"]#,"qm7"
    analysis = tune.run(
        training_function,
        config={
            "dataname": tune.grid_search(["ogbg-molhiv"]), #,,",,"qm7","ogbg-molhiv","NCI1", DD,ogbg-molhiv,"NCI1","ZINC_full","IMDB-MULTI"]),#DD, Mutagenicity,"NCI1",'Cora', 'PubMed']),  # for ray tune, need abs path ogbg-molhiv
            ...
            "batch_size":tune.grid_search([2560,5120]),#args.batch_size]),
            "epochs": tune.grid_search([200]),
            "head": tune.grid_search([4]),####no gat no head
            "nhid": tune.grid_search([64]),
            "dropout_prob": tune.grid_search([0.0,0.1,0.5]),
